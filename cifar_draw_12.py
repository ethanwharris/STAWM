...
class GlimpseDecoder(nn.Module):
    def __init__(self, h, w):
        ...
        self.conv3 = InverseBlock(64, 3, last=True)
...
class CifarDraw(nn.Module):
    def __init__(self, count, memory_size, output_stages=False):
        ...
        self.memory = Memory(output_inverse=True, hidden_size=memory_size, memory_size=memory_size, glimpse_size=12, g_down=1024, c_down=2304, context_net=ContextNet(), glimpse_net=GlimpseNet())
        ...
        if output_stages:
            self.square = visualise.red_square(12, width=1).unsqueeze(0).cuda()
...
def draw(count, memory_size, file, device='cuda'):
    ...
    base_dir = os.path.join('cifar_' + str(memory_size), "12")
