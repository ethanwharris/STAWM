...
CANVAS = torchbearer.state_key('canvas')
TARGET = torchbearer.state_key('target')
...
class Sketchpad(nn.Module):
    def __init__(self, count, memory_size, output_stages=False):
        ...
        self.class_down = nn.Linear(1024, memory_size)
    ...
    def forward(self, x, state=None):
        ...
        class_query = F.relu6(self.class_down(c_data))
        ...
        x = self.memory(class_query)
        classification = self.soft(self.classifier(x))

        if state is not None:
            state[CANVAS] = canvas.sigmoid()
            state[TARGET] = image
...
def draw(count, memory_size, file, device='cuda'):
    ...
    testloader = torch.utils.data.DataLoader(testset, pin_memory=True, batch_size=1,
                                             shuffle=True, num_workers=10)

    base_dir = os.path.join('mnist_sketch_' + str(memory_size), "8")

    model = Sketchpad(count, memory_size, output_stages=True)
    ...
    from visualise import StagesGrid, WrongImages

    w = WrongImages('wrong_images.png', key=CANVAS, target=torchbearer.Y_TRUE, pred=torchbearer.Y_PRED, pad_value=1, nrow=25, num_images=25)
    w2 = WrongImages('target_images.png', key=TARGET, target=torchbearer.Y_TRUE, pred=torchbearer.Y_PRED, pad_value=1, nrow=25, num_images=25)

    trial = Trial(model, optimizer, nn.CrossEntropyLoss(reduction='sum'), ['loss'], pass_state=True, callbacks=[
        callbacks.TensorBoardImages(comment=current_time, nrow=10, num_images=20, name='Prediction', write_each_epoch=True,
                                    key=CANVAS, pad_value=1),
        callbacks.TensorBoardImages(comment=current_time + '_mnist', nrow=10, num_images=20, name='Target', write_each_epoch=False,
                                    key=TARGET, pad_value=1),
        w,
        w2,
        StagesGrid('sketchpad_stages.png', STAGES, 20)
...
def run(count, memory_size, iteration, device='cuda'):
    ...
    model = Sketchpad(count, memory_size)
    ...
    def mse_loss(state):
        return F.mse_loss(state[CANVAS], state[TARGET], reduction='sum')

    trial = Trial(model, optimizer, nn.CrossEntropyLoss(reduction='sum'), ['acc', 'loss'], pass_state=True, callbacks=[
